...
CONV_GRID_SIZES   = [16, 32, 64, 128]  # resolutions for convergence study
...
def run_convergence() -> tuple:
    ...
    Reference is Nx = 2 × max(CONV_GRID_SIZES).

    Returns
    -------
    grid_sizes : list[int]
    dofs       : np.ndarray  (Nx²)
    errors     : np.ndarray  (L2 vs reference)
    walltimes  : np.ndarray  (seconds for CONV_N_STEPS steps)
    """
    Nx_ref = max(CONV_GRID_SIZES) * 2
...
def run_frontier() -> dict:
    ...
    For each Nx in CONV_GRID_SIZES run FRONTIER_N_STEPS and record:
      - uniform wallclock
      - uniform L2 error (vs Nx=256 reference)
...
def plot_frontier(frontier: dict):
    ...
    ax.semilogy(aw, ae, "s--", color="#ffa502", lw=2, ms=8, label="Uniform + AMR overhead")

    # Annotate resolution
    for Nx, w, e in zip(gs, uw, ue):
        ax.annotate(f"Nx={Nx}", (w, e), textcoords="offset points",
                    xytext=(-6, 8), fontsize=8, color="#4a90d9", ha="right")
    for Nx, w, e in zip(gs, aw, ae):
        ax.annotate(f"Nx={Nx}", (w, e), textcoords="offset points",
                    xytext=(6, -12), fontsize=8, color="#c47a00", ha="left")
...
def write_summary(conv, timing, frontier):
    ...
    lines = [
        ...
        "=" * 60,
        "",
        f"Convergence study  (n_steps={CONV_N_STEPS}, ref Nx={max(grid_sizes)*2})",
    ...
    lines += [
        ...
        "",
        f"Efficiency frontier  (n_steps={FRONTIER_N_STEPS}, ref Nx={max(CONV_GRID_SIZES)*2})",
        f"{'Nx':>6}  {'uniform wall':>13}  {'uniform L2':>12}  "
        f"{'amr wall':>10}  {'amr L2':>12}",
        "-" * 60,
    ]
    for Nx, uw, ue, aw, ae in zip(
        frontier["grid_sizes"],
        frontier["uniform_wall"], frontier["uniform_err"],
        frontier["amr_wall"],     frontier["amr_err"],
    ):
        lines.append(
            f"{Nx:6d}  {uw:12.3f}s  {ue:12.4e}  {aw:9.3f}s  {ae:12.4e}"
        )
    lines += ["", "=" * 60]
